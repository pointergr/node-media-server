...
// Τα segments του HLS: 2s το καθένα, 10 στη λίστα. Κοινά σε όλες τις περιπτώσεις,
// γι' αυτό γράφονται μία φορά.
const HLS = [
  "-f", "hls",
  "-hls_time", String(SEGMENT_SEC),
  // 10 segments = 20s παράθυρο. Δεν αλλάζει το latency (ο player συγχρονίζεται
  // στο live edge, όχι στην αρχή της λίστας) — αλλάζει το περιθώριο: θεατής που
  // κόλλησε έως 20s κάνει rebuffer και συνεχίζει αντί να πέσει έξω από το
  // παράθυρο και να ξαναμπεί με κενό, και τα ff/*.ts της αναδίπλωσης (r2.js)
  // μένουν στον δίσκο αρκετά ώστε να τα προλάβει και ο αργός θεατής — το
  // delete_segments σβήνει ό,τι βγει από τη λίστα. Το κόστος είναι μόνο λίγα
  // MB δίσκου ανά stream.
  "-hls_list_size", "10",
