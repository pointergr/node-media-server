...
export class ClientsController {
  ...
  create(@Body() body: Partial<CreateClientDto>) {
    if (!body.name || !body.serverId) throw new BadRequestException('name και serverId απαιτούνται');
  ...
  @Post(':id/paths')
  addPath(@Param('id', ParseIntPipe) id: number, @Body() body: { path?: string }) {
    ...
    return this.clients.addPath(id, body.path);
  }
