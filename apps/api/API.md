...
|---|---|---|---|
| **API key** (`pk_…`) | `/admin/apikeys` στο panel, ή `apikey.js "<υπηρεσία>"` στον host | ποτέ (ανάκληση από την ίδια οθόνη ή `apikey.js revoke`) | εξωτερικές υπηρεσίες — **αυτό θέλεις** |
...
από key. Δύο εξαιρέσεις, και οι δύο για τον ίδιο λόγο — ένα key δεν έχει
λογαριασμό: το `PATCH /auth/me` (401) και τα `/apikeys` (403), ώστε ένα κλειδί
που διέρρευσε να μη φτιάχνει διαδόχους πριν προλάβεις να το ανακαλέσεις.
