...
- Το `limit` είναι **αθροιστικό σε όλα τα paths του πελάτη** (το πακέτο πουλιέται ανά
  πελάτη, όχι ανά κάμερα)· `0` ή απόν = χωρίς όριο. Με panel, το νούμερο είναι **παράγωγο**:
  το άθροισμα των πακέτων του πελάτη (δες `apps/api/README.md`) — εδώ φτάνει έτοιμο. Ο θεατής πάνω από το όριο παίρνει 404
...
- **Ο πελάτης** βλέπει μόνο τα δικά του streams (`/`): stream key έτοιμο για αντιγραφή στο
  OBS, μετρητή θεατών σε σχέση με το όριό του, player, κατάσταση εκπομπής («εκπέμπει
  01:23», bitrate εισόδου) και γραφήματα 24ώρου για θεατές και bitrate. Τα δικά του και
  μόνο: το `GET /me/series` φιλτράρει με το `clientId` του token και δεν δίνει CPU/μνήμη
  του μηχανήματος — οι `/servers/:host/*` οθόνες μένουν admin.
