...
### Πακέτα (μόνο με panel)

Με το κεντρικό panel το `limit` δεν το γράφεις στο χέρι: είναι **παράγωγο των πακέτων** του
πελάτη. Κάθε πακέτο έχει δύο νούμερα — μέγιστους θεατές και μέγιστα streams — και ο πελάτης
μπορεί να πάρει πολλά, ακόμα και το ίδιο πολλές φορές:

| Πακέτο | Server | Θεατές | Streams |
|---|---|---|---|
| `basic` ×2 | `stream1` | 50 | 2 |
| `extra` ×1 | `stream1` | 100 | 1 |
| **σύνολο στον `stream1`** | | **200** | **5** |

- Τα όρια είναι `Σ (ποσότητα × όριο πακέτου)`, **ανά server**. Πελάτης **χωρίς πακέτο**
  βγάζει `0`, δηλαδή χωρίς όριο — η ίδια σημασία που έχει ήδη το `0` παραπάνω. Γι' αυτό και
  τα νούμερα του πακέτου είναι ≥1: ένα `0` μέσα σε άθροισμα θα σήμαινε «100 + 0 = απεριόριστο».
- **Ο server είναι του πακέτου, όχι του πελάτη.** Κάθε πακέτο δηλώνει σε ποιον stream server
  πέφτουν οι επόμενες αγορές του. Ο πελάτης που αγόρασε «basic» όσο το basic έδειχνε
  `stream1` μένει εκεί για πάντα· όταν ο `stream1` γεμίσει και βάλεις το basic να δείχνει
  `stream2`, **καμία** παλιά αγορά δεν μετακομίζει — εκεί πάει μόνο η επόμενη. Ένας πελάτης
  μπορεί έτσι να έχει streams και όρια σε δύο μηχανήματα ταυτόχρονα, και το κάθε path
  ανήκει σε ένα από αυτά.
- **Ο stream server δεν ξέρει τι είναι πακέτο.** Παίρνει έτοιμο `limit` στο `clients.json`,
  με το σχήμα που έχει πάντα — όλη η αριθμητική γίνεται στο `apps/api`.
- Το όριο **streams** μετράει paths (πόσα μπορεί να έχει ο πελάτης) και επιβάλλεται μόνο
  στο panel, τη στιγμή που προστίθεται path (409). Paths που υπάρχουν ήδη δεν κόβονται αν
  αργότερα μικρύνει το πακέτο, και ο stream server δεν το βλέπει καθόλου αυτό το όριο.
- Πακέτο που το κρατάει έστω ένας πελάτης δεν διαγράφεται (409) — αλλιώς θα άλλαζαν σιωπηλά
  τα όριά του.
...
- **Ο admin** βλέπει `/admin/servers` (προσθήκη/διαγραφή stream server, εδώ παράγεται και το
  `panel: {...}` block για το `config.json` κάθε server), `/admin/packages` (τα πακέτα που
  πουλάς), `/admin/clients` (πελάτες, πακέτα, paths, κλειδιά) και `/admin` (live streams
...
- **Ο πελάτης** βλέπει μόνο τα δικά του streams (`/`): stream key έτοιμο για αντιγραφή στο
  ...
  **συνολικά** σε σχέση με το όριό του (το όριο είναι του λογαριασμού στον server της κάθε
  εκπομπής, όχι της κάθε κάμερας) και γραφήματα 24ώρου για θεατές, είσοδο και έξοδο. Τα δικά του και μόνο: το
