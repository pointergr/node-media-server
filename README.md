...
απόλυτα URLs του R2 (`-hls_base_url`). Το πρώτο segment δημοσιεύεται αμέσως από το
DNS-only origin. Κάθε επόμενο segment αρχίζει να ανεβαίνει μόλις εμφανιστεί, ενώ προς τους
θεατές δημοσιεύεται η προηγούμενη έκδοση του playlist. Έχει έτσι έναν ολόκληρο κύκλο 2s
για να φτάσει στο R2, με σταθερό κόστος ενός segment επιπλέον live latency.

Στην επόμενη αλλαγή του source playlist, ό,τι έχει ανέβει κρατάει το απόλυτο R2 URL· ό,τι
δεν πρόλαβε ακυρώνεται και γίνεται `ff/<segment>.ts`, δηλαδή σερβίρεται απευθείας από το
DNS-only origin. Η ανανέωση του playlist δεν περιμένει ποτέ Promise δικτύου: το R2 είναι
βελτιστοποίηση της κίνησης, όχι εξάρτηση της αναπαραγωγής.

Τα PUT τρέχουν σε ουρά έως 4 ταυτόχρονα ανά stream, με μία μόνο προσπάθεια και προθεσμία
ενός segment. Jobs που βγήκαν από το live window απορρίπτονται και το `stop()` ακυρώνει
όσα βρίσκονται ήδη στο δίκτυο. Αν το τοπικό αρχείο έχει επίσης χαθεί, δεν δημοσιεύεται
ψεύτικο `ff/` URL: αφαιρείται από το playlist, αυξάνεται ανάλογα το media sequence και το
επόμενο διαθέσιμο segment παίρνει `#EXT-X-DISCONTINUITY`.
