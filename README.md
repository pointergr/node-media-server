...
| Πακέτο | Θεατές | Streams |
|---|---|---|
| `basic` ×2 | 50 | 2 |
| `extra` ×1 | 100 | 1 |
| **σύνολο** | **200** | **5** |

- Τα όρια είναι `Σ (ποσότητα × όριο πακέτου)`. Πελάτης **χωρίς πακέτο** βγάζει `0`, δηλαδή
  χωρίς όριο — η ίδια σημασία που έχει ήδη το `0` παραπάνω. Γι' αυτό και τα νούμερα του
  πακέτου είναι ≥1: ένα `0` μέσα σε άθροισμα θα σήμαινε «100 + 0 = απεριόριστο».
...
- **Ο πελάτης** βλέπει μόνο τα δικά του streams (`/`): stream key έτοιμο για αντιγραφή στο
  ...
  **συνολικά** σε σχέση με το όριό του (το όριο είναι του λογαριασμού, όχι της κάθε
  κάμερας) και γραφήματα 24ώρου για θεατές, είσοδο και έξοδο. Τα δικά του και μόνο: το
