...
- Το `server` block του snapshot είναι ο **μόνος** δρόμος του panel προς το μηχάνημα — δεν
  υπάρχει ssh από το `/admin` και δεν θέλουμε (shell πίσω από JWT). Γι' αυτό κουβαλάει και
  φόρτο (`load` **μαζί** με `cpus`: το «3» είναι άνεση σε 8 πυρήνες και πνιγμός σε 2), δίσκο
...
Η **αναστολή** φαίνεται και στο API, όχι μόνο στο sync: το `/me/*` γίνεται
**read-only** όσο ο πελάτης ή το πακέτο είναι σε αναστολή (`me.controller#writable`) —
οι αναγνώσεις δουλεύουν και δείχνουν `suspended`, τα γραψίματα απαντούν 403. Αλλιώς η
εκπομπή έπεφτε σε ≤10s αλλά ο πελάτης συνέχιζε να φτιάχνει streams και να ανανεώνει
κλειδιά.
...
με τη μία διαφορά που δεν έχει το shell: το `humanOnly` (`auth/human-only.ts`) κόβει τα
`pk_` από αυτά τα endpoints (το `sub` τους είναι αρνητικό), αλλιώς ένα κλειδί που
διέρρευσε γεννάει διαδόχους και η ανάκληση δεν τελειώνει ποτέ. Το ίδιο `humanOnly`
κόβει και τα `restart`/kill session: το provisioning φτιάχνει πελάτες, δεν ρίχνει
εκπομπές. Δεν είναι scopes — είναι οι δύο δρόμοι που καμία υπηρεσία δεν χρειάζεται και
που κοστίζουν περισσότερο όταν το κλειδί διαρρεύσει.
Τρία ακόμα που κρατάνε όρθιο το auth και δεν φαίνονται από τα endpoints: το
**`JWT_SECRET` δεν έχει fallback** (`auth/secret.ts` — τυχαίο ≥32 χαρακτήρων, αλλιώς η
διεργασία δεν ξεκινάει· fallback σημαίνει server που υπογράφει admin tokens με μυστικό
που διαβάζεται από το repo, σιωπηλά), το **login φρενάρει** μετά από 10 συνεχόμενες
αποτυχίες ανά IP+username (`auth/throttle.ts`, μόνο αποτυχίες — η επιτυχία μηδενίζει,
ώστε να μην κόβεται πελάτης που δουλεύει· το scrypt είναι πια ασύγχρονο, αλλιώς κάθε
προσπάθεια κρατούσε τον event loop), και τα **μυστικά του stream server** (`token` του
sync, `adminPass`) φεύγουν **μόνο** στην απάντηση του `POST /servers` — κανένα `GET` δεν
τα ξαναδίνει, γιατί το `GET` το κάνει και κάθε API key. Πάνω σε αυτά, το `POST
