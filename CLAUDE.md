...
**Ο πελάτης δεν έχει server — τον έχει η αγορά.** Το `Package` δηλώνει πού πέφτουν οι
**επόμενες** αγορές του· η κάθε γραμμή `ClientPackage` κρατάει στιγμιότυπο αυτού του
`serverId` τη στιγμή που έγινε (γι' αυτό το serverId είναι και μέρος του κλειδιού της:
«1× basic στον stream1» και «1× basic στον stream2» είναι δύο γραμμές του ίδιου πελάτη).
Αλλάζοντας τον server ενός πακέτου δεν μετακομίζει κανείς — γεμίζει ο stream1, το basic
δείχνει stream2, και εκεί πάει μόνο ο επόμενος που θα αγοράσει. Συνέπεια: **όλα είναι ανά
server** — όρια (`maxViewersOf(packages, serverId)`), paths (το `Path.serverId` διαλέγεται
από τις αγορές του πελάτη) και το sync (ο κάθε server παίρνει μόνο τους πελάτες που έχουν
αγορά ή path εκεί, με τα δικά του paths). Ο πελάτης χωρίς καμία αγορά δεν ανήκει πουθενά:
τον βάζει ο διαχειριστής όπου θέλει και μένει χωρίς όριο, ο ίδιος δρόμος αναδίπλωσης του
`0`.
