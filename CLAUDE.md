...
**Λίστα, μετά καρτέλα.** Οι τρεις κατάλογοι του admin (πελάτες, πλάνα, servers)
είναι `index.vue` + `[id].vue`: η λίστα δείχνει μία γραμμή ανά εγγραφή με τρία
κουμπιά (είσοδος ως πελάτης / στατιστικά, επεξεργασία, διαγραφή) και όλη η
επεξεργασία ζει στη σελίδα της μίας εγγραφής. Ο λόγος δεν είναι αισθητικός: με
είκοσι κάρτες ανοιχτές μαζί κάθε «Αποθήκευση» ήταν σε λάθος κάρτα ένα scroll
μακριά, και ένα `v-model` ανά πεδίο ανά εγγραφή σήμαινε `Record<number, …>` για
κάθε τι που πληκτρολογείται. Το φίλτρο και η σελιδοποίηση είναι **στον browser**
(`app/composables/usePaged.ts`, δοκιμάζεται από το `test-paged.js`): οι τρεις
κατάλογοι έρχονται ούτως ή άλλως ολόκληροι σε μία κλήση, ένα `?page=` θα ζητούσε
αλλαγή σε τρία endpoints για κάτι που δεν έχει σπάσει. Οι φόρμες δημιουργίας
μένουν στη λίστα, κλειστές πίσω από το «Νέος …». Ο player του πελάτη είναι ανά
stream, μέσα στην καρτέλα του — ίδιο modal με το `/admin`, γιατί εκεί το «ποιο
