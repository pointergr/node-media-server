...
| playlists | τα ίδια αρχεία | τα γράφει το `r2.js` ένα επίπεδο πάνω, **μετά** τα uploads |

Η σειρά upload-πριν-publish είναι η ουσία: ανάποδα ο player ζητάει segment που δεν έχει ανέβει. Τα playlists μένουν πάντα στο origin — εκεί μετριούνται οι θεατές.

Ένας κανόνας για όλες τις περιπτώσεις: ό,τι `*.m3u8` βρεθεί στο `ff/` δημοσιεύεται με **το
ίδιο όνομα** στον φάκελο του stream (γι' αυτό ο ffmpeg γράφει εκεί με τα τελικά ονόματα, όχι
με `ff.m3u8`). Το master (`#EXT-X-STREAM-INF`) αντιγράφεται αυτούσιο — δείχνει σε *σχετικά*
