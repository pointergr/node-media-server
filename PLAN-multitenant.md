...
Κατάσταση: **Φάσεις 1-5 και 7 υλοποιήθηκαν, Φάση 6 υλοποιήθηκε στο `apps/api`
(2026-08-13).** Γραμμένο 2026-08-13.
...
**Απόκλιση στην υλοποίηση:** ο διακόπτης της επιβολής είναι η **ύπαρξη** του αρχείου, όχι
η απουσία του περιεχομένου του όπως λέει η παράγραφος παραπάνω. Αρχείο που λείπει ή
χάλασε = δρόμος αναδίπλωσης (όπως εδώ)· ένα **έγκυρο** `{}` όμως σημαίνει «ο server δεν
έχει κανέναν πελάτη» και μπλοκάρει τους πάντες, γιατί αλλιώς ένας server που μόλις μπήκε
στο panel, ή του οποίου απενεργοποιήθηκαν όλοι οι πελάτες, θα γινόταν ορθάνοιχτος με το
πρώτο sync. Δες `apps/stream/config.js`.
...
**Έγινε.** Το ανοιχτό ερώτημα για το άγνωστο path αποφασίστηκε — δες «Ανοιχτά ερωτήματα».
...
**Έγινε, στο `apps/api`.** Πλήρες συμβόλαιο στο [apps/api/README.md](apps/api/README.md).
...
**Έγινε, με μία προσθήκη πέρα από το πλάνο:** το `passwords.js` φτιάχνει και πελάτη
`default` για το `/live/stream` στην πρώτη εγκατάσταση. Χρειάστηκε λόγω της Απόκλισης
παραπάνω (η ύπαρξη του `clients.json` είναι ο διακόπτης) — χωρίς αυτό, ένας καθαρός
server με `auth.publish: false` και ακόμα χωρίς αρχείο θα ήταν ορθάνοιχτος σε όποιον
ξέρει το URL, πριν προλάβει κανείς να δηλώσει πελάτες στο panel.
...
- **Άγνωστο path**: μπλόκο ή ελεύθερο; **Αποφασίστηκε: μπλόκο.** (δες Φάση 2 —
  επηρεάζει το πότε μπορεί να γίνει αυστηρός ο έλεγχος).
