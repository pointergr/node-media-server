...
**Έγινε, με δύο αποκλίσεις από το πλάνο:**
- το workspace package μετονομάστηκε σε `@pointer/stream` (ήταν `node-media-server`) — με το παλιό όνομα το
  npm symlink-άρει τον ίδιο τον workspace πάνω στο ομώνυμο dependency και το πραγματικό
  library επιβιώνει μόνο χάρη στο nested `node_modules`.
- το `npm ci` του `install` δεν τρέχει μέσα στο `apps/stream` αλλά από τη ρίζα με
  `-w apps/stream`, γιατί με workspaces το `package-lock.json` ζει μόνο εκεί.
...
**Έγινε, νωρίτερα απ' ό,τι προέβλεπε το πλάνο** — πριν υπάρχει Nuxt να το αντικαταστήσει
(Βήμα 3 δεν έγινε). Μέχρι να φτιαχτεί το panel UI, η διαχείριση γίνεται με curl/seed (δες
